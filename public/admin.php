...
// All other actions require admin login
requireAdmin();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($modAction === 'add_admin') {
        ...
        if (!filter_var($newEmail, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($stmt->fetch()) {
                ...
                $stmt = $db->prepare("INSERT INTO admins (email, password_hash, role, created_at) VALUES (?, ?, ?, NOW())");
                $stmt->execute([$newEmail, $hash, $newRole]);
    ...
    unset($_SESSION['csrf_token']);
    redirect(url('admin.php?action=' . ($action === 'dashboard' ? '' : $action)));
}
...
if (in_array($action, ['dashboard', 'admins', 'audit', 'password', 'top_reacted'])) {
...
if (!in_array($filterStatus, $validStatuses) && !$isTopReacted) {
    $filterStatus = 'pending';
}

// Build query with reaction counts
$where = [];
$qParams = [];
...
if ($adminFilterLang && in_array($adminFilterLang, ['en', 'sw'])) {
    $where[] = "m.language = ?";
    $qParams[] = $adminFilterLang;
}

if ($adminFilterCat && in_array($adminFilterCat, getCategories())) {
    $where[] = "m.category = ?";
    $qParams[] = $adminFilterCat;
}
...
$query = "SELECT m.*,
    ...
    COALESCE(rc.helped_count, 0) AS helped_count,
    COALESCE(rc.hope_count, 0) AS hope_count,
    COALESCE(rc.not_alone_count, 0) AS not_alone_count
FROM messages m
LEFT JOIN (
    SELECT message_id,
        COUNT(*) AS reaction_count,
        SUM(CASE WHEN type = 'helped' THEN 1 ELSE 0 END) AS helped_count,
        SUM(CASE WHEN type = 'hope' THEN 1 ELSE 0 END) AS hope_count,
        SUM(CASE WHEN type = 'not_alone' THEN 1 ELSE 0 END) AS not_alone_count
    FROM reactions
    GROUP BY message_id
...
// Find the most reacted message ID for highlighting
$topMsgId = 0;
if (!empty($messages)) {
    $topCount = 0;
    foreach ($messages as $msg) {
        if ($msg['reaction_count'] > $topCount) {
            $topCount = $msg['reaction_count'];
            $topMsgId = $msg['id'];
        }
    }
    if ($topCount === 0) $topMsgId = 0;
}

// --- AUDIT LOG ---
$auditLog = [];
if ($action === 'audit') {
    $stmt = $db->query("SELECT al.*, a.email, m.content FROM audit_log al JOIN admins a ON al.admin_id = a.id JOIN messages m ON al.message_id = m.id ORDER BY al.created_at DESC LIMIT 100");
...
$adminsList = [];
if ($action === 'admins') {
    $adminsList = $db->query("SELECT id, email, role, created_at FROM admins ORDER BY created_at ASC")->fetchAll();
}
...
<div class="container">
    <div class="admin-layout">
        ...
        <aside class="admin-sidebar">
            ...
            <a href="<?= url('admin.php') ?>" class="<?= $action === 'dashboard' ? 'active' : '' ?>">
                <?= e(__('admin_pending')) ?> (<?= $pendingCount ?>)
            </a>
            <a href="<?= url('admin.php?action=approved') ?>" class="<?= $action === 'approved' ? 'active' : '' ?>">
                <?= e(__('admin_approved')) ?> (<?= $approvedCount ?>)
            </a>
            <a href="<?= url('admin.php?action=rejected') ?>" class="<?= $action === 'rejected' ? 'active' : '' ?>">
                <?= e(__('admin_rejected')) ?> (<?= $rejectedCount ?>)
            </a>
            <a href="<?= url('admin.php?action=top_reacted') ?>" class="<?= $action === 'top_reacted' ? 'active' : '' ?>">
                <?= e(__('admin_top_reacted')) ?>
            </a>
            <a href="<?= url('admin.php?action=audit') ?>" class="<?= $action === 'audit' ? 'active' : '' ?>">
                <?= e(__('admin_audit')) ?>
            </a>
            ...
            <hr style="margin: 1rem 0; border-color: var(--border-light);">
            <a href="<?= url('admin.php?action=admins') ?>" class="<?= $action === 'admins' ? 'active' : '' ?>">
                <?= e(__('admin_manage_admins')) ?>
            </a>
            <a href="<?= url('admin.php?action=password') ?>" class="<?= $action === 'password' ? 'active' : '' ?>">
                <?= e(__('admin_change_password')) ?>
            </a>
            <hr style="margin: 1rem 0; border-color: var(--border-light);">
        ...
        <div class="admin-content">
            ...
            <div class="stat-cards">
                <div class="stat-card">
                    ...
                    <div class="stat-label"><?= e(__('admin_pending')) ?></div>
                </div>
                <div class="stat-card">
                    <div class="stat-number"><?= $approvedCount ?></div>
                    <div class="stat-label"><?= e(__('admin_approved')) ?></div>
                </div>
                <div class="stat-card">
                    <div class="stat-number"><?= $rejectedCount ?></div>
                    <div class="stat-label"><?= e(__('admin_rejected')) ?></div>
                </div>
                <div class="stat-card">
                    <div class="stat-number"><?= $totalReactions ?></div>
                    <div class="stat-label"><?= e(__('admin_total_reactions')) ?></div>
                </div>
            </div>
            ...
            <?php if ($action === 'password'): ?>
                ...
                <div class="card" style="max-width: 450px;">
                    <form method="POST" action="<?= url('admin.php?action=password') ?>">
                        ...
                        <div class="form-group">
                            <label><?= e(__('admin_current_password')) ?></label>
                            <input type="password" name="current_password" class="form-control" required>
                        </div>
                        <div class="form-group">
                            <label><?= e(__('admin_new_password')) ?></label>
                            <input type="password" name="new_password" class="form-control" required minlength="6">
                            <p class="form-hint"><?= e(__('admin_pass_hint')) ?></p>
                        </div>
                        <div class="form-group">
                            <label><?= e(__('admin_confirm_password')) ?></label>
                    ...
                    <form method="POST" action="<?= url('admin.php?action=admins') ?>">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr 150px auto; gap: 0.75rem; align-items: end;">
                            <div class="form-group" style="margin-bottom: 0;">
                                <label><?= e(__('admin_email')) ?></label>
                                <input type="email" name="new_email" class="form-control" required placeholder="email@example.com">
                            </div>
                            <div class="form-group" style="margin-bottom: 0;">
                                <label><?= e(__('admin_password')) ?></label>
                                <input type="password" name="new_password" class="form-control" required minlength="6" placeholder="Min 6 chars">
                            </div>
                            <div class="form-group" style="margin-bottom: 0;">
                                <label><?= e(__('admin_role_label')) ?></label>
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 0.5rem;">
                            <div>
                                ...
                                <?php if ((int)$adm['id'] === (int)$_SESSION['admin_id']): ?>
                                    <span class="badge badge-approved" style="margin-left: 0.25rem;"><?= e(__('admin_you')) ?></span>
                                <?php endif; ?>
                                <p class="text-muted" style="font-size: 0.8rem; margin-top: 0.25rem;">
                                    <?= e(__('admin_joined')) ?>: <?= date('M j, Y', strtotime($adm['created_at'])) ?>
                                </p>
                            </div>
                            <?php if ((int)$adm['id'] !== (int)$_SESSION['admin_id']): ?>
                                <form method="POST" action="<?= url('admin.php?action=admins') ?>" onsubmit="return confirm('<?= e(__('admin_delete_confirm')) ?>');">
                                    <?= csrfField() ?>
                                    <input type="hidden" name="mod_action" value="delete_admin">
                    ...
                    <?php foreach ($auditLog as $log): ?>
                        <div class="admin-message">
                            <div class="message-meta">
                                <span class="badge badge-<?= $log['action'] === 'approved' ? 'approved' : ($log['action'] === 'rejected' ? 'rejected' : 'pending') ?>"><?= e($log['action']) ?></span>
                                <span class="text-muted"><?= e($log['email']) ?></span>
                                <span class="message-date"><?= date('M j, Y H:i', strtotime($log['created_at'])) ?></span>
                            </div>
                            <p class="message-content" style="font-size: 0.9rem; margin-top: 0.5rem;">
                                <?= e(mb_substr($log['content'], 0, 150)) ?><?= mb_strlen($log['content']) > 150 ? '...' : '' ?>
                            </p>
                            <?php if ($log['details']): ?>
                                <p class="form-hint"><?= e($log['details']) ?></p>
                            <?php endif; ?>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
            ...
            <?php else: ?>
                ...
                <form class="filters" method="GET" action="<?= BASE_URL ?>/admin.php">
                    ...
                    <select name="filter_cat" onchange="this.form.submit()">
                        ...
                        <?php foreach (getCategories() as $cat): ?>
                            <option value="<?= $cat ?>" <?= $adminFilterCat === $cat ? 'selected' : '' ?>><?= e(__('cat_' . $cat)) ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <?php if (empty($messages)): ?>
                    <div class="card text-center" style="padding: 2rem;">
                        <p class="text-muted"><?= e(__('admin_no_pending')) ?></p>
                    </div>
                <?php else: ?>
                    <?php foreach ($messages as $msg): ?>
                        <div class="admin-message<?= ($topMsgId === (int)$msg['id'] && $msg['reaction_count'] > 0) ? ' top-reacted' : '' ?>">
                            <div class="message-meta">
                                ...
                                <?php if ($topMsgId === (int)$msg['id'] && $msg['reaction_count'] > 0): ?>
                                    <span class="badge badge-top-reacted"><?= e(__('admin_most_reacted_badge')) ?></span>
                                <?php endif; ?>
                                <span class="message-date"><?= date('M j, Y H:i', strtotime($msg['created_at'])) ?></span>
                            </div>
                            <div class="message-content <?= $msg['format'] === 'quote' ? 'quote' : '' ?>" style="margin-top: 0.75rem;">
                                <?= e($msg['content']) ?>
                            </div>
                            ...
                            <div class="admin-actions">
                                <?php if ($msg['status'] !== 'approved'): ?>
                                    <form method="POST" action="<?= url('admin.php?action=' . $action) ?>" style="display:inline;">
                                        <?= csrfField() ?>
                                        <input type="hidden" name="mod_action" value="approve">
                                        <input type="hidden" name="message_id" value="<?= $msg['id'] ?>">
                                        <button type="submit" class="btn btn-success btn-sm"><?= e(__('admin_approve')) ?></button>
                                    </form>
                                <?php endif; ?>

                                <button class="btn btn-secondary btn-sm" onclick="document.getElementById('edit-<?= $msg['id'] ?>').style.display='block'">
                                    <?= e(__('admin_edit')) ?>
                                </button>
                                ...
                                <?php if ($msg['status'] !== 'rejected'): ?>
                                    <form method="POST" action="<?= url('admin.php?action=' . $action) ?>" style="display:inline;">
                                        ...
                                        <input type="hidden" name="message_id" value="<?= $msg['id'] ?>">
                                        <button type="submit" class="btn btn-danger btn-sm"><?= e(__('admin_reject')) ?></button>
                                    </form>
                                <?php endif; ?>
                            </div>
                        </div>
